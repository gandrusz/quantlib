...
namespace QuantLib {
    ...
    boost::shared_ptr<CatSimulation> EventSet::newSimulation(const Date& start, const Date& end) const{
        return boost::shared_ptr<CatSimulation>(new EventSetSimulation(events_, eventsStart_, eventsEnd_, start, end));
    }
    ...
    boost::shared_ptr<CatSimulation> BetaRisk::newSimulation(const Date& start, const Date& end) const {
        return boost::shared_ptr<CatSimulation>(new BetaRiskSimulation(start, end, maxLoss_, lambda_, alpha_, beta_));
    }
}